...
if __name__ == '__main__':
    ...
    # Remove CUDA
    # model.cuda()  # Commented out to run on CPU
    ...
    for epoch in range(args.epochs):
        ...
        for user, item, label in train_loader:
            # Move data to device
            user = user  # Use .cuda() if using GPU
            item = item  # Use .cuda() if using GPU
            label = torch.tensor(label).float()  # Convert to tensor and float type
        ...
        print("The time elapse of epoch {:03d} is: {}".format(epoch, time.strftime("%H:%M:%S", time.gmtime(elapsed_time))))
